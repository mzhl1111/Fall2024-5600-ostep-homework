# 1 
  no difference for the two type
  response will be 0, 200, 400
  turnaround will be 200, 400, 600
```
...
# 2
No difference as 100, 200 300 in order
...
```
python scheduler.py -l 100,200,300 -p RR -q 1 -c
ARG policy RR
ARG jlist 100,200,300
...
# 4
 - all jobs have same length
 - all jobs ordered by length in ascending order

 # 5
 - all jobs have same length as Quantum
 - all jobs have different length but all less then Quantum

 # 6
 - length for a task increase will not affect its respond time, but all following job expected a longer respond time

```
Final statistics:
  Job   0 -- Response: 0.00  Turnaround 150.00  Wait 0.00
  Job   1 -- Response: 150.00  Turnaround 350.00  Wait 150.00
  Job   2 -- Response: 350.00  Turnaround 650.00  Wait 350.00
```
vs 
...
Final statistics:
  Job   0 -- Response: 0.00  Turnaround 100.00  Wait 0.00
  Job   1 -- Response: 100.00  Turnaround 350.00  Wait 100.00
  Job   2 -- Response: 350.00  Turnaround 650.00  Wait 350.00
```

# 7
 - Small Quantum: more context switches, will have super short respond time but longer turnover (like Q3)
 - large Quantum: fewer context switches, will more like FIFO, and super large Quantum make the behavior same as FIFO
 - if we mark the job number from 0 to N
   then the worst case response time for each job will Q * N
